...
if( !class_exists( 'Guten_Learn_Customizer' ) ){
	class Guten_Learn_Customizer extends Guten_Learn_Helper{
		...
		*/
		public static function add_custom_control( $control ){
			...
			if( !is_array( $control ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Control. Must be an array with "type" & "class" key.', 'guten-learn' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $control[ 'type' ] ) ){
				$err = esc_html__( 'Custom Control must contain "type" key ( type of control ).', 'guten-learn' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $control[ 'class' ] ) ){
				$err = esc_html__( 'Custom Control must contain "class" key ( name of custom class ).', 'guten-learn' );
				self::add_error( $err );
				$flag = false;
			}		

			if( !isset( $control[ 'sanitize' ] ) ){
				$err = esc_html__( 'Custom Control must contain "sanitize" key ( a callback function ).', 'guten-learn' );
				self::add_error( $err );
				$flag = false;
			}
		...
		*/
		public static function add_custom_section( $section ){
			...
			if( !is_array( $section ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Section. Must be an array with "type" & "class" key.', 'guten-learn' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $section[ 'type' ] ) ){
				$err = esc_html__( 'Custom Section must contain "type" key ( type of section ).', 'guten-learn' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $section[ 'class' ] ) ){
				$err = esc_html__( 'Custom Section must contain "class" key ( name of section class ).', 'guten-learn' );
				self::add_error( $err );
				$flag = false;
			}		
				...
				foreach( $args[ 'fields' ] as $field ){
					...
					if( isset( $field[ 'id' ] ) ){
						...
						if( array_key_exists( $field_id, self::$settings ) ){
							...
							$msg = sprintf( esc_html__( 'Field with ID: %s already exists.', 'guten-learn' ), $field[ 'id' ] );
							self::add_error( $msg );
						}
		...
		*/
		private static function validate_argument( $args ){
			...
			if( !is_array( $args ) ){
				...
				$msg = esc_html__( 'You must pass array as an argument in set method.', 'guten-learn' );
			...
			if( !isset( $args[ 'section' ] ) ){
				...
				$msg = esc_html__( 'No section found.', 'guten-learn' );
				self::add_error( $msg, 2 );
			...
			if( !is_array( $args[ 'section' ] ) ){
				...
				$msg = sprintf( 
					...
					esc_html__( 'Invalid argument', 'guten-learn' ),	
					esc_html__( 'section', 'guten-learn' ),
					esc_html__( 'Must be a an array with atleast an id, title...', 'guten-learn' )
				);
				...
				$msg = esc_html__( 'Section id is missing.', 'guten-learn' );
				self::add_error( $msg, 2 );
			}
			...
			if( isset( $args[ 'fields' ] ) && !is_array( $args[ 'fields' ] ) ){
				...
				$msg = sprintf( 
					'%s <strong><em>%s</em></strong>. %s',
					esc_html__( 'Invalid argument', 'guten-learn' ),	
					esc_html__( 'fields', 'guten-learn' ),
					esc_html__( 'Must be a an array', 'guten-learn' )
				);
						...
						<p>
							<?php  
								echo sprintf( '<strong>%s</strong> %s', 
									esc_html__( 'Gute Learn Customizer:', 'guten-learn' ),
									esc_html( $value ) 
								);  
							?>
						</p>
		...
		*/
		private static function get_panel_arg( $panel ){
			...
			$args = array(
			    ...
			    esc_html__( 'No Title Specified.', 'guten-learn' ) : 
			    $panel['title'],
			);
		...
		*/
		private static function get_control_arg( $section, $field, $prefix_on_section = true ){
			...
			if( isset( $field[ 'active_callback' ] ) ) {
				...
				if( is_array( $acb ) ){
	            	$args[ 'active_callback' ] = $acb[ 'fn_name' ];
				}else{
		...
		*/
		private static function get_section_arg( $panel_id = false, $section ){
			$args = array(
				'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'guten-learn' ) : $section[ 'title' ],
			);
...
add_action( 'customize_register', array( Guten_Learn_Customizer::get_instance(), 'register' ) );

if( !function_exists( 'guten_learn_get' ) ):
	/**
	 ...
	 * @package Guten Learn WordPress Theme
	 */
	function guten_learn_get( $id ){
		$id = Guten_Learn_Helper::with_prefix( $id );
		return Guten_Learn_Customizer::get( $id );
	}
