 ...
 */
function guten_learn_responsive_device_css(){
	foreach( array( 'md' => 'desktop', 'sm' => 'tablet', 'xs' => 'mobile' ) as $size => $device ){

		Guten_Learn_Css::add_styles( array(
			#Typography
			array(
			...
			array(
				'selector'  => Guten_Learn_Helper::with_prefix_selector( '%s-post .post-title a' ),
				'props'		=> array(
					'font-size' => 'post-title-size-' . $device,
				),
			),
			array(
				'selector' => Guten_Learn_Helper::with_prefix_selector( '%s-main-menu > ul > li > a' ),
				'props'	   => array(
			...
			array(
				'selector' => Guten_Learn_Helper::with_prefix_selector( '%s-inner-banner-wrapper' ),
				'props' => array(
...
add_action( 'init', 'guten_learn_responsive_device_css' );
