...
add_action( 'init', 'guten_learn_custom_width', 99 );
add_action( 'customize_preview_init', 'guten_learn_custom_width', 150 );
 ...
 * @package Guten Learn WordPress Theme
 */
function guten_learn_custom_width(){
	// echo guten_learn_get( 'container-width' );die;
	if( 'default' == guten_learn_get( 'container-width' ) ) :
		# container width
		$style = array(
			array(
				'selector' => '.container',
		...
		Guten_Learn_Css::add_styles( $style, 'md' );
	endif;

}

add_action( 'init', 'guten_learn_all_device_css' );
/**
 ...
 * @package Guten Learn WordPress Theme
 */
function guten_learn_all_device_css(){

	$style = array(
		# Primary Color
		array(
			'selector' => Guten_Learn_Helper::with_prefix_selector( '.pagination .nav-links > *.current, ::selection, %s-main-menu > ul > li > a:after, %s-btn-primary, #infinite-handle span, ul.wc-block-grid__products li.wc-block-grid__product button, ul.wc-block-grid__products li.wc-block-grid__product .wp-block-button__link, ul.wc-block-grid__products li.wc-block-grid__product button:hover, ul.wc-block-grid__products li.wc-block-grid__product .wp-block-button__link:hover, ul.wc-block-grid__products li.wc-block-grid__product .wc-block-grid__product-onsale, .woocommerce ul.products li.product .button, .woocommerce ul.products li.product .added_to_cart.wc-forward,
		...
		array(
			'selector'  => Guten_Learn_Helper::with_prefix_selector( '%s-main-menu > ul > li > a, %s-search-icons, %s-search-icons:visited, %s-main-menu div > ul li a' ),
			'props'		=> array(
		...
		# inner banner
		array(
			'selector' => Guten_Learn_Helper::with_prefix_selector( '%s-inner-banner-wrapper:after' ),
			'props'    => array(
				'background-color' => 'ib-background-color'
			)
		),
		array(
			'selector' => Guten_Learn_Helper::with_prefix_selector( '%s-inner-banner-wrapper %s-inner-banner .entry-title' ),
			'props'    => array(
		...
		array(
			'selector'  => '.banner-slider-content .inner-banner-btn a',
			'props'		=> array(
				'background' => 'primary-color'
			)
		),
		array(
			'selector'  => '.gutenlearn-bottom-header-wrapper',
			'props'		=> array(
