...
//Lets decide what is the title
	if ( is_search() ) {
		...
		$_title = sprintf( esc_html( _n( '%1$d resultados de búsqueda para "%2$s"', '%1$d resultados de búsqueda para "%2$s"', $count, 'posterity' ) ), $count, get_search_query() );
	} elseif ( is_archive() ) {
		if ( is_author() ) {
			...
			$_title = sprintf( esc_html__( '%s', 'posterity' ), '<span>' . single_cat_title( '', false ) . '</span>' );
		} elseif ( is_tag() ) {
