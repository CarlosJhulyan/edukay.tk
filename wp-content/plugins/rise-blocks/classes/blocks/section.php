		...
		public function prepare_scripts_styles(){
			...
			foreach( $blocks as $block ){
				...
				foreach( [ 'mobile', 'tablet', 'desktop' ] as $device ){
					...
					$css = array(
						array(
							'selector' => self::add_prefix( '> .%prefix-section-wrapper' ),
							'props' => array_merge( $padding[ $device ], $width )
						),
						array(
							'selector' => self::add_prefix( '> .%prefix-section-wrapper .%prefix-section-shape svg' ),
					...
					if( isset( $attrs[ 'shapePosition' ] ) && $attrs[ 'shapePosition' ] == 'top' && !$verticalFlip ){
						$css[] = array(
							'selector' => self::add_prefix( '> .%prefix-section-wrapper .%prefix-section-shape' ),
							'props' => array(
								'margin-bottom' => array(
				...
				if( ( !isset( $attrs[ 'bgType' ] ) || 'image' == $attrs[ 'bgType' ] ) && isset( $attrs[ 'bgImage' ] ) && $attrs[ 'bgImage' ][ 'url'  ] != '' ){
					...
					if( isset( $attrs[ 'bgOverlay' ] ) ){
						$css[] = array(
							'selector' => self::add_prefix( '> .%prefix-section-wrapper > .%prefix-section-overlay' ),
							'props' => array(
